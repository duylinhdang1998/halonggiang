...
export function mountCharacter(host:HTMLElement,onAnchors:(anchors:Anchors)=>void) {
 ...
 const scene=new THREE.Scene();const camera=new THREE.PerspectiveCamera(32,1,.1,60);const target=new THREE.Vector3(0,2.75,0);
 const model=createGiangModel();scene.add(model);
