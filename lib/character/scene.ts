...
export function mountCharacter(host:HTMLElement,onAnchors:(anchors:Anchors)=>void,onStatus:(status:'ready'|'error')=>void) {
 ...
 host.appendChild(renderer.domElement);const canvas=renderer.domElement;host.dataset.model="blender-continuous-upper";
 ...
 const model=createGiangModel(()=>{dirty=true;onStatus('ready');},()=>onStatus('error'));scene.add(model);
